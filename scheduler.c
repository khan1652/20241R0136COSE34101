...
#define ALGORITHM_NUM 6 //알고리즘 추가 시 업데이트
...
void EvaluateOne(process** p){
    ...
    //각 프로세스 정보 출력
    for (int i = 0; i < processNum; i++) {
        printf("Pid: %d, Arrival Time: %d, CPU Burst: %d, I/O Burst: %d, Priority: %d, Waiting Time: %d, Turnaround Time: %d\n",
               p[i]->pid, p[i]->arrivalTime, p[i]->CPUburst, p[i]->IOburst, p[i]->priority, p[i]->waitingTime, p[i]->turnaroundTime);
    }
    printf("\n");
    ...
    avgWaiting /= processNum;
    avgTurnaround /= processNum;
...
void RR(){
    ...
    //RR 알고리즘 (FCFS with time quantum)
    //preemption 시 현재 진행중이던 프로세스 제외한 레디큐의 프로세스 중 정렬하여 선택
    int readyCount = 0;
    int waitingCount = 0;
    int currentTime = 0;
    int finished = 0;
