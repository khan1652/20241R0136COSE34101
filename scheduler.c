...
char *algorithms[ALGORITHM_NUM] = {"First Come First Served", "Non-Preemptive Shortest Job First", "Preemptive Shortest Job First", "Non-Preemptive Priority", "Preemptive Priority", "Round Robin", "Non-preemptive Longest I/O Shortest Job First Algorithm", "Preemptive Longest I/O Shortest Job First Algorithm"}; 
...
void sort_IO(process** processes, int num){
    ...
    //sort processes (I/O burst time 큰 순서로, I/O burst time 같을 시 CPU remaining burst time 작은 순서로, CPU remaining burst time 같을 시 arrival time이 작은 순서로, arrival time도 같으면 pid가 작은 순서로)
    for (int i = 0; i < num - 1; i++) {
        for (int j = 0; j < num - i - 1; j++) {
            if (processes[j]->IOburst < processes[j + 1]->IOburst || 
                (processes[j]->IOburst == processes[j + 1]->IOburst &&
                 (processes[j]->CPUremaining > processes[j + 1]->CPUremaining ||
                  (processes[j]->CPUremaining == processes[j + 1]->CPUremaining &&
                   (processes[j]->arrivalTime > processes[j + 1]->arrivalTime ||
                    (processes[j]->arrivalTime == processes[j + 1]->arrivalTime &&
...
void NonPreemptive_LIOSJF(){
    printf("<Non-preemptive Longest I/O Shortest Job First Algorithm>\n");
    //Config: readyQueue, waitingQueue, p 설정
    process** readyQueue = malloc(sizeof(process*) * processNum);
    if (readyQueue == NULL) {
        printf("malloc error");
    ...
    while (finished < processNum) {
        ...
        sort_IO(readyQueue, readyCount); //I/O burst time, CPU burst time으로 정렬
...
void Preemptive_LIOSJF(){
    printf("<Preemptive Longest I/O Shortest Job First Algorithm>\n");
    //Config: readyQueue, waitingQueue, p 설정
    process** readyQueue = malloc(sizeof(process*) * processNum);
    if (readyQueue == NULL) {
        printf("malloc error");
    ...
    while (finished < processNum) {
        ...
        if (runningProcess != NULL) { //작업 중인 프로세스 존재함
            if(readyCount > 0){ //레디큐 다시 확인해서 필요시 preemption
                if((runningProcess->IOburst < readyQueue[0]->IOburst) || (runningProcess->IOburst == readyQueue[0]->IOburst && runningProcess->CPUremaining > readyQueue[0]->CPUremaining)){ 
                    readyQueue[readyCount++] = runningProcess; //기존 프로세스를 레디큐에 다시 넣기
                    runningProcess = readyQueue[0];
                    for (int j = 1; j < readyCount; j++) { //새 프로세스를 레디큐에서 제거
                        readyQueue[j - 1] = readyQueue[j];}
...
int main(int argc, char **argv){
    ...
    NonPreemptive_LIOSJF(); //추가적인 알고리즘
    Preemptive_LIOSJF();
